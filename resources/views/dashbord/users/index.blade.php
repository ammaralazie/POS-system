                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if (auth()->user()->hasPermission('create_users'))
                                    <a href="{{ route('dashbord.users.create') }}" class="btn btn-primary"><i
                            ...
                            <thead>
                                ...
                                @foreach ($users as $index => $user)
                                    <tr>
                                        ...
                                        <th>{{ $user->email }}</th>
                                        <th><img src="{{ $user->image }}" alt="" style="width: 40px;height:40px;
                                border-radius: 40px;"></th>
                                        @if (Auth()->user()->hasPermission('delete_users'))
                                            <th><a class="btn btn-info btn-sm"
                                                    href="{{ route('dashbord.users.edit', ['id' => $user->id]) }}">@lang('site.edit')</a>
                                            @else
                                            <th><a class="btn btn-info btn-sm disabled" href="#">@lang('site.edit')</a>
                                        @endif

                                        @if (Auth()->user()->hasPermission('delete_users'))
                                            <form action="{{ route('dashbord.users.destroy', ['id' => $user->id]) }}"
